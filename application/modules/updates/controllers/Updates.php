...
//        $this->db_upd_tables($src['data']);
    ...
    function mysql_backup() {
        ...
        if (!write_file('./resource/backup/database-full-backup_' . date('Y-m-d') . '.zip', $backup)) {
            $this->session->set_flashdata('response_status', 'error');
            $this->session->set_flashdata('message', 'Database backup failed cannot write to /resource/backup folder.');
            redirect('updates');
        }
    ...
    public function delete_file($file_name = NULL){
        ...
        if(file_exists($fullpath)){ unlink($fullpath); redirect($_SERVER['HTTP_REFERER']); }else{
            Applib::go_to($_SERVER['HTTP_REFERER'],'error',lang('operation_failed'));
        }
    }
